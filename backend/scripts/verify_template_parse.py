...
import os
# Ensure project root (backend folder) is on sys.path when running as script
...
def main():
    try:
        from data_management.templates import get_template
        from data_management.utils import FileParser, OPENPYXL_AVAILABLE
    except Exception:
    ...
    for module in modules:
        ...
        if OPENPYXL_AVAILABLE:
            try:
                ...
                wb = Workbook()
                ws = wb.active
                # Sanitize sheet title for Excel (max 31 chars, no [:]\/*?[]) characters)
                safe_title = (template.display_name or module)[:31]
                for ch in [':', '\\', '/', '?', '*', '[', ']']:
